//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:athletix/api.dart';
import 'package:test/test.dart';

// tests for FilterTopProductsServices200ResponseData
void main() {
  // final instance = FilterTopProductsServices200ResponseData();

  group('test FilterTopProductsServices200ResponseData', () {
    // FilterTopProductsServices200ResponseDataProducts products
    test('to test the property `products`', () async {
      // TODO
    });

    // FilterTopProductsServices200ResponseDataServices services
    test('to test the property `services`', () async {
      // TODO
    });


  });

}
